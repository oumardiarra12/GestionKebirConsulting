        ...
        <div class="row g-4">
            ...
            <div class="col-lg-8">
                    ...
                    <div class="row g-4 align-items-center">
                        ...
                        <div class="col-md-4 text-center">
                            ...
                            @if ($photo_candidat && Storage::disk('public')->exists($photo_candidat))
                                <img src="{{ asset('storage/' . $photo_candidat) }}" alt="Photo du candidat" class="rounded-circle shadow-sm" style="width: 130px; height: 130px; object-fit: cover;">
                            @else
                                <div class="rounded-circle bg-light d-flex align-items-center justify-content-center text-muted shadow" style="width: 130px; height: 130px;">
                        ...
                        <div class="col-md-8">
                            ...
                            <div class="row g-2 text-dark small">
                                @foreach ($infos as $label => $valeur)
                                    <div class="col-sm-6">
                                        <span class="fw-semibold text-muted">{{ $label }} :</span> {{ $valeur ?? '—' }}
                                    </div>
                                @endforeach
                            </div>
                        </div>
                ...
                <div class="card border-0 shadow-sm mt-4 p-4">
                    <h5 class="fw-bold text-dark mb-3"><i class="fas fa-file-alt me-2 text-primary"></i>Curriculum Vitae</h5>
                    @if ($cv_candidat)
                        <a href="{{ asset('storage/' . $cv_candidat) }}" target="_blank" class="text-decoration-none text-primary">
                    ...
                    <div class="mb-4">
                        ...
                        <ul class="ps-3 mb-0">
                            @forelse($abonne->formations as $formation)
                                <li class="mb-2">
                                    <strong>{{ $formation->diplome_formation }}</strong> – {{ $formation->etablissement_formation }}
                                    <small class="text-muted d-block">({{ $formation->date_debut_formation }} - {{ $formation->date_fin_formation }})</small>
                                </li>
                            @empty
                    ...
                    <div class="mb-4">
                        ...
                        <ul class="ps-3 mb-0">
                            @forelse($abonne->experiences as $exp)
                                <li class="mb-2">
                                    <strong>{{ $exp->poste_experience }}</strong> chez {{ $exp->entreprise_experience }}
                                    <small class="text-muted d-block">({{ $exp->date_debut_experience }} - {{ $exp->date_fin_experience }})</small>
                                </li>
                            @empty
                    ...
                    <div class="mb-4">
                        ...
                        <ul class="ps-3 mb-0">
                            @forelse($abonne->langues as $langue)
                                <li>{{ $langue->nom_langue }} <span class="text-muted">({{ $langue->pivot->niveau }})</span></li>
                            @empty
...
<div class="card shadow-sm mb-4">
    <div class="card-body">
        ...
        <ul class="list-unstyled mb-0 text-dark small">
            <li class="mb-2"><i class="fas fa-user me-2 text-muted"></i><strong>Nom :</strong> {{ $nom_candidat }} {{ $prenom_candidat }}</li>
            <li class="mb-2"><i class="fas fa-calendar me-2 text-muted"></i><strong>Naissance :</strong> {{ $datenaissance_candidat }}</li>
            <li class="mb-2"><i class="fas fa-venus-mars me-2 text-muted"></i><strong>Sexe :</strong> {{ $sexe_candidat }}</li>
            <li class="mb-2"><i class="fas fa-phone me-2 text-muted"></i><strong>Téléphone :</strong> {{ $tel_candidat }}</li>
            <li class="mb-2"><i class="fas fa-envelope me-2 text-muted"></i><strong>Email :</strong> {{ $email }}</li>
            <li class="mb-2"><i class="fas fa-map-marker-alt me-2 text-muted"></i><strong>Adresse :</strong> {{ $adresse_candidat }}</li>
        </ul>
    </div>
</div>
...
@if ($cv_candidat)
    <div class="card shadow-sm">
        <div class="card-body text-center">
            ...
            <a href="{{ asset('storage/' . $cv_candidat) }}" class="btn btn-success btn-sm mt-2" download>
